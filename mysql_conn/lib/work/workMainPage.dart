...
class _workMainState extends State<workMain> {
  ...
  Map<DateTime, List<Event>> vaEvents = {
    DateTime(2024, 7, 11): [Event(DateTime(2024, 7, 11), "Sample Event")],
    // 더 많은 이벤트 추가
  };
          ...
          content: Column(
            ...
            children: <Widget>[
              ...
              DropdownButtonFormField<String>(
                ...
                decoration: InputDecoration(
                  labelText: '매장명을 선택하세요',
                  enabledBorder: OutlineInputBorder(
                    borderRadius: BorderRadius.circular(10.0),
                  ),
                ),
                items: storeInfoList.map<DropdownMenuItem<String>>((Map<String, dynamic> store) {
                  return DropdownMenuItem<String>(
                    // value: '${store['channel']} ${store['store']}',
                    // child: Text('${store['channel']} ${store['store']}'),
                    value: store['code'], // Use the store code as the value
                    child: Text('${store['channel']} ${store['store']}'),
                  );
                }).toList(),
              ...
              DropdownButtonFormField<String>(
                ...
                items: <String>['진열','재고확인', '매대확인', '행사점검', '기타']
          ...
          actions: <Widget>[
            ...
            TextButton(
              ...
              onPressed: () async {
                ...
                String? content = storeName;
                DateTime? vDate = _selectedDay;
                // 메모 추가
                //await addMemo(title, content);
                await addVisitStore(title!, content!,vDate!);

                setState(() {
                  // 방문매장 등록
                  print("add today store");
                  //getMemoList();
                });
                Navigator.of(context).pop();
              },
